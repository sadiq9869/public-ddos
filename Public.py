...
OVERLORD_MAX_DURATION = 180  # Max attack duration for overlords
PAID_USER_MAX_DURATION = 180  # Max attack duration for paid users
...
def welcome_start(message):
    ...
    if len(command) > 1:
        ...
        if referrer_id in referral_data and referrer_id != user_id:
            ...
            if user_data[referrer_id].get('referral_bonus', 0) < MAX_REFERRAL_BONBegins:
    ...
    bot.reply_to(message, 
                 ...
                 f"👤 **View Your Profile:** /myinfo 📊📈\n"
                 f"🛠️ **Full Command List:** /help 📖📋\n"
                 f"\n💎 **Want {OVERLORD_MAX_DURATION}s Attack Duration? Upgrade to Thunder Arsenal! (Free: {USER_MAX_DURATION}s)** 💎⏰⏱️\n"
                 f"{PRICE_LIST}",
                 parse_mode="Markdown")
...
def handle_attack(message):
    ...
    try:
        ...
        result = subprocess.run(full_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=200)
        bot.reply_to(message, 
                     f"✅ **Attack Command Executed Successfully!** ✅✔️\n"
                     f"📜 **Output:** {result.stdout.decode()}\n"
                     f"{branded_footer()} 😍🤗")
    except subprocess.TimeoutExpired:
        bot.reply_to(message, 
                     f"❌ **Attack Timed Out!** ❌🚫\n"
                     f"⚠️ **Command took too long to execute!** ⚠️⏰\n"
                     f"🛠️ **Need Help? Contact Overlord {SUPPORT_CONTACT}** 📞🤗"
                     f"{branded_footer()} 😓😢")
        if is_overlord:
            with overlord_attack_lock:
